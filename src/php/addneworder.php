...
if (!empty($userId) && !empty($amount)) {
    ...
    try {
        ...
        $query = $dbcon->prepare('INSERT INTO tilaus (astunnus, tilauspvm, summa, tila) 
        VALUES (:astunnus, :tilauspvm, :summa, :tila)');
        ...
        $data = array('tilausnro' => $dbcon->lastInsertId(), 'astunnus' => $userId, 'tilauspvm' => $date, 'summa' => $amount, 'tila' => $status);
